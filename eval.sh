...
#Text retrieval
python eval_ir_llms.py --mode=test --media=txt --model_name=Open-Orca/Mistral-7B-OpenOrca --use_llm_score=True --mocheg_result_path=checkpoints/mocheg_txt_results/query_result_txt.pkl --prompt="Is this corpus related to the query? Answer with yes or no."
...
python FNdetection_llm.py --media=multimodal --model_name=llava-hf/llava-v1.6-mistral-7b-hf --evidence_type=gold --two_level_prompting=False --task1_out=retrieval/output/ir_llms/mocheg/txt --task1_out_img==retrieval/output/ir_llms/mocheg/img --prompt="Does this evidence support or refute this claim?answer with yes if it supports, answer with no if it refutes and answer with none if it does not provide enough information."
