...
def read_news(data_path, content="all", skip_empty_text_evidence=False,
              paragraph_sent_num=None):  # text_evidence, all, txt, img
    news_dict = {}
...
def load_corpus_df(data_folder, corpus_name='Corpus3_sentence_level.csv'):
    ...
    if corpus_name == "Corpus3_sentence_level.csv":
        ...
        collection_filepath = os.path.join(data_folder, corpus_name)
    ...
    for _, row in df_news.iterrows():
        ...
        if claim_id in news_dict:
            ...
            if skip_empty_text_evidence and pd.isna(row["Evidence"]):
                ...
                example = News(claim_id, row["Snopes URL"], "", row['Claim'], row['cleaned_truthfulness'],
                               row['Origin'], row["ruling_outline"])
                evidence = row["Evidence"]
                if not pd.isna(evidence) and len(evidence) > 0:
                    evidence = evidence.replace("<p>", "")
                    evidence = evidence.replace("</p>", "")
